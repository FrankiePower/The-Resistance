...
## 2026-02-23 - Success #4: Full Local Unlimited E2E Pass

- Goal: Verify complete one-command local flow with unlimited Soroban limits.
- Status: `SUCCESS`
...
### Output highlights
- Deployed local GameHub: `CA7YMAIRMGRY42JVRB3NFOHS737QLZZWPCIG4AXOOHQPRCPVYXNRSJXQ`
- Deployed local The Resistance: `CBP4X6BLOEPFFKJGQFPTP6KREKV6P3U7KSQIPE2OPOKMRGDKPWGWAYMA`
- `start_game` tx: `3231aa5537f9a2a8729002bb05f0406f88eca129668fecaa0ee6f4a3b1f0dec4`
- `execute_action` tx: `b99a8dd980d47b110eb98357780a67fd7bd817bfee93dc32cf3fe5618e8abd88`
- Final status: `PASS: end-to-end onchain proof flow succeeded.`

## 2026-02-23 - Success #5: Frontend Local RPC `allowHttp` Fix

- Goal: Fix browser crash when using local quickstart RPC (`http://localhost:8000/soroban/rpc`).
- Status: `SUCCESS`
...
### Root cause
- Generated contract client and direct `rpc.Server` usage did not set `allowHttp: true` for local HTTP RPC.

### Changes
- Updated `the-resistance-frontend/src/games/the-resistance/theResistanceService.ts`
  - Added `ALLOW_HTTP = RPC_URL.startsWith('http://')`
  - Passed `allowHttp: ALLOW_HTTP` to all `new TheResistanceClient(...)` calls.
- Updated `the-resistance-frontend/src/utils/ledgerUtils.ts`
